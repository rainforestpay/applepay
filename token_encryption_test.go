...
func TestECDHESharedSecret(t *testing.T) {
	Convey("Generic test", t, func() {
		// Values taken from https://github.com/processout/applepay/issues/22
		k1 := []byte{
			42, 154, 0, 223, 84, 8, 55, 162, 117, 24,
			142, 136, 128, 33, 143, 69, 194, 175, 251, 116,
			183, 49, 190, 0, 228, 232, 21, 229, 203, 187,
			226, 96,
		}
		k2 := []byte{
			195, 5, 124, 117, 229, 83, 2, 62, 251, 203,
			46, 195, 218, 215, 152, 7, 226, 64, 180, 63,
			19, 183, 198, 37, 234, 194, 188, 222, 158, 85,
			230, 191,
		}

		privECDH, err := ecdh.P256().NewPrivateKey(k1)
		So(err, ShouldBeNil)

		privECDH2, err := ecdh.P256().NewPrivateKey(k2)
		So(err, ShouldBeNil)
		pubECDH := privECDH2.PublicKey()

		rawKey := pubECDH.Bytes()
		pub := &ecdsa.PublicKey{
			Curve: elliptic.P256(),
		...
		priv := &ecdsa.PrivateKey{
			PublicKey: *pub,
			D:         big.NewInt(0).SetBytes(privECDH.Bytes()),
		}
		...
		So(sharedSecretHex, ShouldEqual, "005363f5f75762476c04d7f32385629501368593df7e89aea4cdaebe4ae6c217")
	})
}
...
func TestDeriveEncryptionKey(t *testing.T) {
	Convey("Arbitrary numbers should give a correct result", t, func() {
		So(
			hex.EncodeToString(
				deriveEncryptionKey(nil, []byte{0}),
			),
			ShouldEqual,
